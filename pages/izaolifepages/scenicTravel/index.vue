        ...
        <view class="header-container" :class="{ expanded: isCategoriesExpanded }">
            ...
            <view class="search-container" @click="onSearch()">
                <view class="search-box">
                    <uni-search-bar placeholder="台儿庄古城" clearButton="none" cancelButton="none" :radius="100"
...
export default {
    data() {
        return {
            ...
            restaurants: mockDATA.travelInfo,
        };
    },
    methods: {
        navigateTo(item) {
            uni.navigateTo({
                url: `/pages/izaolifepages/restaurantDetail/index?id=${item.id}&type=${2}`,
            });
        },
        selectCategory(tab) {
            this.currentTab = tab;
