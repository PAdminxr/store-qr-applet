...
export default {
    ...
    onLoad() {
        ...
        this.statusBarHeight = systemInfo.statusBarHeight - 6;

        // 判断是否支持获取胶囊信息
        if (systemInfo.customButtonBoundingClientRect) {
            const { width, bottom } = systemInfo.customButtonBoundingClientRect;
            this.customBarHeight = bottom; // 胶囊底部位置 = 自定义导航栏总高度
            this.searchMaxWidth = width - 80; // 留出左右各40px间距，避免贴着胶囊
        } else {
            // 不支持的平台模拟一个高度
            this.customBarHeight = this.statusBarHeight + 44;
            this.searchMaxWidth = 200; // 默认最大宽度
        }
